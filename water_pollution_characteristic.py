...
class ModelEnum(Enum):
    WaterPollutionCharacteristic = '水污染特征图谱匹配'
...
def calc_tracing(model: LoadModel, param: AnalysisParam):
    ...
    return (WorkflowParam._build_model_file_output("水质特征图谱", find_files_by_pattern_v2(f'{user_host_path}', '.*.jpg$')),
            WorkflowParam._build_model_file_output("水质光谱特征", find_files_by_pattern_v2(f'{user_host_path}', '.*_光谱特征.*.xlsx$')),
            WorkflowParam._build_model_file_output("水质特征图谱分析报告", find_files_by_pattern_v2(f'{user_host_path}', '.*分析报告.*.pdf$')))

def get_workflow(param: AnalysisParam):
    account = param.account
    # 加载模型
    model = LoadModel(account, ModelEnum.WaterPollutionCharacteristic)
    #
