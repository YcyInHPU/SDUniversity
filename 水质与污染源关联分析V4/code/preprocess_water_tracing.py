...
def Anomaly_Detection(data,WQParams,output_path):
    ...
    plt.title(Site_Name + '水质异常值检测--' + WQParams)
    plt.legend()
    plt.grid()

    # 导出散点图
    plt.savefig(output_path + '/' + Site_Name + '_' + WQParams + '_异常检测.png', dpi = 300)
    plt.savefig('异常检测.png', dpi = 300)
    plt.show()
...
def Report_Generation():
    # 加载模板
    template_path = input_path + '/Report/' + Site_Name + '_' + WQParams + '_水质污染溯源报告模板.docx'  # 文件路径
...
def convert_docx_to_pdf():
    ...
    # word.Visible = False
    # 打开Word文档
    doc_file_name = output_path + '/' + Site_Name + '_' + WQParams + '_水质溯源分析报告.docx'
    # 将文档另存为PDF
    pdf_file_name = output_path + '/' + Site_Name + '_' + WQParams + '_水质溯源分析报告.pdf'
    convert(doc_file_name, pdf_file_name)
    # doc.SaveAs(file_name, FileFormat=17)  # 17表示PDF格式
    # 关闭文档和Word应用程序
    # doc.Close()
    # word.Quit()
...
def word2pdf():
    ...
    :return: word to pdf
    """
    wordPath = output_path + '/' + Site_Name + '_' + WQParams + '_水质溯源分析报告.docx'
    pdfPath = output_path + '/' + Site_Name + '_' + WQParams + '_水质溯源分析报告.pdf'
...
def parse_args():
    ...
    parser.add_argument('--WQParams', default='氨氮', type=str)
...
if __name__ == '__main__':
    ...
    try:
        # 数据时间转换
        data = Data_Time(input_path,Site_Name)[0]
        basin = Data_Time(input_path,Site_Name)[1]
        # 异常值检测
        ...
        for i in range(row_count):
            ...
            pollution_count = Water_quality_trace(monitoring_data, monitoring_time)[2]
            data_list.append(first_column) #搜集每次溯源疑似概率最高的企业
        Enterprise_trace(time_list, data_list) # 企业溯源分析
        # 产汇流
        image_src_folder = input_path + '/Basin/' + Site_Name + '.jpg'
        image_name1 = Province + City + '_' + Site_Name + '_水质与污染源关联分析.jpg'
