# -*- coding: UTF-8 -*- #
"""
@filename:preprocess_enterprise_characteristic.py
@author:Chunyu Yuan
@time:2024-11-26
"""
...
def Report_Generation():
    # 加载模板
    template_path = input_path + '/' + '企业排污报告模板.docx'  # 文件路径
    ...
    image2 = InlineImage(doc, image_path2, width=Mm(140))
    shutil.copy(image_src_folder2, image_path2)
    # 加载图片
    image_path3 = 'image3.png'
    image3 = InlineImage(doc, image_path3, width=Mm(140))
    shutil.copy(image_src_folder3, image_path3)
    # 替换属性
    data = {
    'Province': Province,
    'City': City,
    ...
    doc.render(data)
    # 输出新的文件
    file_name = output_path + '/' + Province + City + Basin2 + '_' + Enterprise + '_行业特征图谱分析报告.docx'
    # 保存新的 Word 文件
    doc.save(file_name)
...
def word2pdf():
    ...
    :return: word to pdf
    """
    wordPath = output_path + '/' + Province + City + Basin2 + '_' + Enterprise + '_行业特征图谱分析报告.docx'
    pdfPath = output_path + '/' + Province + City + Basin2 + '_' + Enterprise + '_行业特征图谱分析报告.pdf'
...
def parse_args():
    ...
    parser.add_argument('--Enterprise', default='养殖', type=str)
...
if __name__ == '__main__':
    ...
    try:
        # 输入源文件夹路径和目标文件夹路径
        image_src_folder = input_path + '/' + Enterprise + '.jpg'
        image_src_folder2 = input_path + '/' + '全行业排污企业数量.jpg'
        image_src_folder3 = input_path + '/' + '全行业排污特征.jpg'
        # 输入图片文件名
        image_name1 = Province + City + Basin2 + '_' + Enterprise + '_行业排污特征图谱.jpg'
        image_name2 = Province + City + Basin2 + '_' + '_全行业排污企业数量.jpg'
        image_name3 = Province + City + Basin2 + '_' + '_全行业排污特征.jpg'
        table_src_folder = input_path + '/' + Enterprise + '.xlsx'
        # 输入表格文件名
        table_name = Province + City + Basin2 + '_' + Enterprise + '_行业排污特征信息.xlsx'
        # 调用函数进行复制
        copy_image(image_src_folder, output_path, image_name1)
        copy_image(image_src_folder2, output_path, image_name2)
        copy_image(image_src_folder3, output_path, image_name3)
